...
export const init = (
  ...
  const component = {
    ...
    onInit: (): void => {
      ...
      serializer.addProperty('peopleSingle', {
        name: 'placeholder',
        category: 'general',
        isLocalizable: true,
      });
    ...
    onAfterRender: async (question: any, el: HTMLElement) => {
      ...
      const element =
        el.getElementsByTagName('kendo-combobox')[0].parentElement;
      if (element) {
      ...
      // Initial selection
      instance.initialSelectionIDs = selectedPersonID;

...
export const createButtonFormGroup = (value: any) => {
  const formGroup = fb.group({
    ...
    field: [
      ...
      value && value.sendMail && value.from === 'field'
        ? Validators.required
        : null,
    ],
    distributionList: [
      get(value, 'distributionList', null),
      value && value.sendMail && value.from === 'distribution'
        ? Validators.required
        : null,
    ],
    templates: [
...
export const createGridActionsFormGroup = (configuration: any) => {
  const formGroup = fb.group({
    ...
    showDetailsLabel: [get(configuration, 'actions.showDetailsLabel', '')],
    showProgress: [get(configuration, 'actions.showProgress', false)],

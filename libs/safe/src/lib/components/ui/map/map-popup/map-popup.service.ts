...
export class SafeMapPopupService {
  ...
  public generatePopupContentTemplate(
    ...
    const title = (title: string, popupTitle?: boolean) =>
      popupTitle
        ? `<h3 class="break-word !m-0 font-bold text-xl">${title}</h3>`
        : `<h3 class="break-word !m-0 font-medium">${title}</h3>`;
    const description = (description: string, popupTitle?: boolean) =>
      popupTitle
        ? `<p class="break-word !m-0 text-gray-600 font-light text-sm">${description}</p>`
        : `<p class="break-word !m-0 mb-2 text-gray-600 font-light">${description}</p>`;

    // Templates use for the property name and the property value to be displayed
    const propertyNameTemplate = (propertyName: string) =>
      `<p class="break-word !m-0 capitalize text-gray-400">${propertyName}</p>`;
    const propertyValueTemplate = (property: any) =>
      `<p class="!m-0 break-word">{{${property}}}</p>`;

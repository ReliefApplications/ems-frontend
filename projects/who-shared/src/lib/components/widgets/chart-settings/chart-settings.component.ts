    ...
    this.tileForm = this.formBuilder.group(
      {
        ...
        source: [(tileSettings && tileSettings.source) ? tileSettings.source : '', Validators.required],
  ...
  getSources(e: any, init?: boolean): void {
    if (e.value === 'resource') {
      this.apollo.query<GetResourcesQueryResponse>({
        ...
        this.sources = res.data.resources.map(source => source = {
          id: source.id,
          name: source.name
        });
        if (!init) {
          this.tileForm.get('source').setValue(null);
          this.tileForm.get('xAxis').setValue(null);
          this.tileForm.get('yAxis').setValue(null);
        }
        this.fields = [];
      });
    } else {

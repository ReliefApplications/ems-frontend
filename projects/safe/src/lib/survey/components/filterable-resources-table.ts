...
export const filterableGridFilterValues =
...
export function init(Survey: any, apollo: Apollo, dialog: MatDialog, formBuilder: FormBuilder): void {
  ...
  let filters: { field: string, operator: string, value: string, questionID: string }[] = [{
    field: '',
    operator: '',
    value: '',
    questionID: ''
  ...
  const component = {
    name: 'filterableResourcesTable',
    title: 'Filterable resources table',
    ...
    onInit: (): void => {
      ...
      Survey
        .JsonObject
        .metaData
        .addProperty('filterableResourcesTable', {
          name: 'Select columns',
    ...
    onLoaded(question: any): void {
      ...
      if (question.resource) {
        if (question.selectQuestion) {
          ...
          if (question.selectQuestion) {
            question.registerFunctionOnPropertyValueChanged('filterCondition',
              () => {
                const resourcesFilters = filterableGridFilterValues.getValue();
                resourcesFilters[0].operator = question.filterCondition;
                filterableGridFilterValues.next(resourcesFilters);
        ...
        getResourceById({id: question.resource}).subscribe(response => {
          ...
          question.value = res.map((r: any) => r.value);
          if (!question.placeholder) {
        ...
        if (question.selectQuestion) {
          if (question.selectQuestion === '#staticValue') {
            ...
            question.survey.onValueChanged.add((survey: any, options: any) => {
              if (options.name === question.selectQuestion) {
                // if (!!options.question.customQuestion || options.value) {
          ...
          let obj = JSON.parse(question.customFilter);
  ...
  const setAdvanceFilter = (value: string, question: string | any) => {
    ...
    if (!filters.some((x: any) => x.field === field)) {
      filters.push({field: question.filterBy, operator: question.filterCondition, value, questionID: question.id});
    } else {
      filters.map((x: any) => {
        if (x.field === field) {
          x.value = value;

...
export class SafeApplicationSummaryComponent implements OnInit {
  ...
  constructor(translate: TranslateService) {
    this.statuses[0].short = translate.instant('status.active')[0].toUpperCase();
    this.statuses[1].short = translate.instant('status.pending')[0].toUpperCase();
    this.statuses[2].short = translate.instant('status.archived')[0].toUpperCase();
  }
